...
6. Uses a pre-allocated 1024-byte writable buffer for `audiocore.WaveFile`, a larger mixer buffer, and a `0.8` software volume level to reduce click/crackle artifacts from SD-card read latency or digital clipping. For more physical output, adjust the Audio BFF gain jumper and use an appropriately rated 4-8Ω speaker.

## WAV playback buffers and click reduction
...
The script also sets the `audiomixer.Mixer` buffer size to `4096` bytes so brief SD-card read delays are less likely to become audible ticks. The mixer voice level is set to `0.8` instead of full scale, leaving digital headroom for loud WAV files that could otherwise clip and sound like clicks or crackles.
